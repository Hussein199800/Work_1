...
function renderRecords() {
    ...
    tbody.innerHTML = records.map(function (record) {
        ...
        let date = "-";
        try { date = new Date(record.createdAt).toLocaleDateString("ar-SY"); } catch (e) {}
...
function docSignatures() {
    return '<footer class="doc-sign">' +
        ...
        '<div><span>الختم</span><i class="stamp-box"></i></div>' +
        "</footer>";
}
...
function buildFinalRecordReport(record, mode) {
    ...
    let createdAt = "-";
    try { createdAt = new Date(record.createdAt).toLocaleDateString("ar-SY"); } catch (e) {}
    ...
    html += '<table class="doc-meta"><tr>' +
        ...
        "<td><span>المندوب</span><strong>" + escapeHTML(record.representativeName) + "</strong></td>" +
    ...
    Object.keys(AGENCY_NAMES).forEach(function (type) {
        ...
        html += "<tr" + (item.count === 0 ? ' class="row-zero"' : "") + "><td>" + AGENCY_NAMES[type] + '</td><td class="num">' + item.count + '</td><td class="num">' + money(item.total) + "</td></tr>";
    });
    ...
    if (detailed) {
        ...
        let sumBase = 0, sumAdd = 0, sumMissing = 0, sumRep = 0, sumGrand = 0;
        ...
        html += '<section class="doc-section">' + secTitle("تفاصيل الوكالات") +
            ...
            '<th class="num">السعر</th><th>الإضافات</th><th class="num">قيمة الإضافات</th>' +
            '<th>الطوابع الناقصة</th><th class="num">المطلوب من المندوب</th>' +
            '<th class="num total-col">الإجمالي<br><small>السعر + الإضافات</small></th><th>ملاحظات</th>' +
            "</tr></thead><tbody>";
        ...
        stats.agencies.slice().sort(function (a, b) { return number(a.number) - number(b.number); }).forEach(function (agency) {
            ...
            const add = isCancelled ? 0 : number(agency.additionsTotal);
            const rep = isCancelled ? 0 : number(agency.representativeAmount);
            const grand = getAgencyGrandTotal(agency);

            sumBase += base; sumAdd += add; sumRep += rep; sumGrand += grand;
            ...
            html += '<tr class="' + (isCancelled ? "row-cancelled" : "") + '">' +
                "<td><strong>" + escapeHTML(agency.number) + "</strong>" + (agency.duplicate ? ' <span class="badge badge-warning">مكررة</span>' : "") + "</td>" +
                ...
                '<td class="small-text">' + (agencyAdditionsToText(agency) === "-" ? "—" : escapeHTML(agencyAdditionsToText(agency))) + "</td>" +
        ...
        html += "</tbody><tfoot><tr>" +
            ...
            '<td class="num">' + money(sumBase) + "</td><td></td>" +
...
/* =========================================================
فتح التقرير كمستند مستقل للطباعة أو الحفظ PDF
(محرك الطباعة في المتصفح يحافظ على الخط العربي والجداول
 بشكل أدق من تحويل الصفحة إلى صورة)
========================================================= */
...
function openReportDocument(bodyHTML, title, options) {
    ...
    if (!win) {
        alert("يرجى السماح بفتح النوافذ المنبثقة لهذا الموقع ثم أعد المحاولة.");
        return;
    }
    ...
    const hint = options.pdf
        ? '<div class="print-hint no-print"><div><strong>حفظ التقرير كملف PDF</strong>' +
          "<span>في نافذة الطباعة اختر من قائمة الطابعة: حفظ كملف PDF، ثم اضغط حفظ.</span></div>" +
          '<button type="button" onclick="window.print()">فتح نافذة الحفظ</button></div>'
        : "";
    ...
    win.document.write(
        ...
        '<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Amiri:wght@700&family=Cairo:wght@400;600;700;800&display=swap">' +
        '<link rel="stylesheet" href="' + cssHref + '">' +
        "<style>@page{size:A4 " + orientation + ";margin:10mm}</style>" +
        '</head><body class="print-doc ' + (options.landscape ? "print-landscape" : "") + '">' +
        hint + bodyHTML +
        "</body></html>"
    );
    win.document.close();
    ...
    const whenReady = function () {
        ...
        if (fonts && fonts.ready) {
            fonts.ready.then(function () { setTimeout(doPrint, 300); });
        } else {
...
function exportReportsToPDF() {
    ...
    if (!container) return;
    openReportDocument(
        '<div class="doc doc-general">' + container.innerHTML + docSignatures() + "</div>",
        "التقرير العام - " + new Date().toISOString().slice(0, 10),
        { pdf: true }
    );
}
...
let html = `
    ...
    ${docLetterhead("التقرير العام", "كل السجلات حتى تاريخ " + todayArabic() + " — " + records.length + " سجل")}
...
function exportRecordToExcel(id) {
    ...
    const muted = [];
    let sBase = 0, sAdd = 0, sMissing = 0, sRep = 0, sGrand = 0;
    ...
    const agenciesSheet = buildStyledTableSheet({
        ...
        subtitle: subtitle,
        headers: ["رقم الوكالة", "النوع", "الحالة", "السعر", "الإضافات", "قيمة الإضافات", "نقص مرافعة", "نقص إسعاف", "نقص معونة", "قيمة الطوابع الناقصة", "المطلوب من المندوب", "الإجمالي (السعر + الإضافات)", "ملاحظات"],
        rows: rows,
        moneyCols: [3, 5, 9, 10, 11],
        mutedRows: muted,
        totalRow: ["الإجمالي", rows.length + " وكالة", "", sBase, "", sAdd,
            stats.missing.pleading, stats.missing.ambulance, stats.missing.aid, sMissing, sRep, sGrand, ""],
        widths: [14, 20, 10, 14, 34, 14, 11, 11, 11, 16, 18, 20, 40]
    });
...
function exportAllRecordsToExcel() {
    ...
    let tBase = 0, tAdd = 0, tMissing = 0, tRep = 0, tAg = 0, tActive = 0, tCancelled = 0, tLost = 0;
    ...
    const rows = records.map(function (record) {
        ...
        tBase += base; tAdd += stats.additionsTotal; tMissing += stats.missingValue; tRep += stats.representativeTotal;
        ...
        return [
            ...
            base, stats.additionsTotal, stats.missingValue, stats.representativeTotal
        ];
    });
    ...
    const recordsSheet = buildStyledTableSheet({
        title: "السجلات",
        subtitle: subtitle,
        headers: ["رقم السجل", "المكتب", "المندوب", "الوكالات", "فعالة", "مفقودة", "ملغاة", "سعر الوكالات", "الإضافات النقدية", "الطوابع الناقصة", "المطلوب من المندوب"],
        rows: rows,
        moneyCols: [7, 8, 9, 10],
        totalRow: ["الإجمالي", "", "", tAg, tActive, tLost, tCancelled, tBase, tAdd, tMissing, tRep],
        widths: [12, 24, 22, 10, 9, 9, 9, 16, 16, 16, 20]
    });
